...
void do_call(int connfd) {
  ...
  if (shm_key == -1) {
    log_print("call ftok error: %s does not exist or if it cannot be accessed by "
              "the calling process\n",
  ...
  if (IS_ERROR == read_request_line(connfd, req_buf, recv_buffer, send_buffer)) {
    goto end;
  }
...
int read_request_line(int connfd, struct req_line *req_line, char *recv_buffer, char *send_buffer) {
  unsigned char c = 0;
  int ret = 0;
...
void response(int connfd, int status_code, const char *reason_phrase, char *send_buffer, const void *message,
              size_t m_size) {
  int send_num = 0;
  send_num = snprintf(send_buffer, BUFFSIZE, "HTTP/1.1 %d %s%s", status_code, reason_phrase, CRLF);
...
void req_parser(int connfd, struct req_line *req_buffer, char *send_buffer) {
  ...
  if (i > 0) {
    ...
    if (fd == -1) {
      const char *mesg = strerror(errno);
      if (errno == EACCES)
        response(connfd, 403, "require permissions", send_buffer, mesg, strlen(mesg));
      else
...
void server(u_int16_t port) {
  ...
  if (serverfd == -1)
    err_sys("call socket function error");
  ...
  if (-1 == setsockopt(serverfd, SOL_SOCKET, SO_REUSEADDR, &on, (socklen_t)sizeof(on)))
    err_sys("setsockopt SO_REUSEADDR error");

  if (-1 == bind(serverfd, (const struct sockaddr *)&serveraddr, (socklen_t)sizeof(serveraddr)))
    err_sys("bind socket fd error");
  if (-1 == listen(serverfd, 0))
    err_sys("listen socket error");

  while (1) {
    if ((connfd = accept(serverfd, (struct sockaddr *)&clientaddr, (socklen_t *)&len)) < 0) {
      if (errno == EINTR)
        continue;
...
int main(int argc, char **argv) {
  ...
  if (shm_key == -1)
    err_sys("SHARE_MEMORY_FILE_PATH does not exist or if it cannot be accessed by "
            "the calling process\n");
  int shm_id = shmget(shm_key, SHM_SIZE, IPC_CREAT | IPC_EXCL | 0660);
  if (shm_id == -1)
    err_sys("call shmget error");
